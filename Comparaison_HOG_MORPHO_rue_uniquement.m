...
dirFond = 'dataSetFondRGB_64_128 - Rue';
...
for n = 1:nbTest
    ...
    if (exist(['dataSetFondRGB_64_128 - Rue\',fondName],'file') == 2) & (indexFond <= nbFondApprentissage)
...
for numImg = 1:nbTestAleatoire
    if round(rand()) == 1 % Chargement d'un pieton
        ...
        numFond = ceil(rand()*size(dir(dirFond),1));
        fondName = ['fond_',num2str(numFond,'%0.4d'),'.jpeg'];
        while(exist(['dataSetFondRGB_64_128 - Rue\',fondName],'file') ~= 2)
            numFond = ceil(rand()*size(dir(dirFond),1));
            fondName = ['fond_',num2str(numFond,'%0.4d'),'.jpeg'];
        end
...
indexFond = find(rep == 0);

nbrTotalPieton = sum(rep);
nbrTotalFond = nbTestAleatoire-sum(rep);

% HOG
VraiPositif = nnz(repTestHOGandSVM(indexPieton))/nbrTotalPieton*100
FauxNegatif = nnz(repTestHOGandSVM(indexFond))/nbrTotalFond*100
VraiNegatif = (nbrTotalPieton-nnz(repTestHOGandSVM(indexPieton)))/nbrTotalPieton*100
FauxPositif = (nbrTotalFond-nnz(repTestHOGandSVM(indexFond)))/nbrTotalFond*100

% Morpho
VraiPositif = nnz(repTestMorpho(indexPieton))/nbrTotalPieton*100
FauxNegatif = nnz(repTestMorpho(indexFond))/nbrTotalFond*100
VraiNegatif = (nbrTotalPieton-nnz(repTestMorpho(indexPieton)))/nbrTotalPieton*100
FauxPositif = (nbrTotalFond-nnz(repTestMorpho(indexFond)))/nbrTotalFond*100